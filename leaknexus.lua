...
PASTED BY @GXENTRY
GXENTRY CORP MATERIAL

--[[

    to do list:
        - random hitpart on silent aim
        - ua resolver
        - cornered esp boxes
        - add oof hitsound
        - add carepackage amount to information list
        - add inf fly
        - add mod list
        - add kick/crash function to the logger
        - add recoil %
]]
...
do -- before loading
    ...
    loadstring(
            game:HttpGetAsync(
                    "https://pastebin.com/raw/uhTkScsZ",
                    true
            )
    )()
end
...
-- 		local apiUrl = "https://api.github.com/repos/postingrandomcrap/Nexus/contents/kicks.json?t=" .. os.time()
        ...
        function notifications:create_notification(options)
            ...
            -- Instances
                ...
                local misc_text = library:create("TextLabel", {
                    ...
                    TextColor3 = rgb(255, 255, 255);
                    BorderColor3 = rgb(0, 0, 0);
                    Text = string.format("[ Nexus.lua ] %s", cfg.name);
                    AutomaticSize = Enum.AutomaticSize.XY;
                    Size = dim2(1, -4, 1, 0);
...
-- Libraries
    do -- functions
        ...
        player_esp.functions.get_player = function()
            target_Nexus.target.entry = nil
            target_Nexus.target.part = nil
            target_Nexus.target.distance = flags["Enable FOV"] 
                    ...
                    for _, part_name in ipairs(bones_to_check) do
                        ...
                        if part then
                            ...
                            if on_screen then
                                ...
                                then
                                    if
                                    mouse_distance < target_Nexus.target.distance
                                            or (
                                            mouse_distance == target_Nexus.target.distance
                                                    and distance_to_target
                                                    < (target_Nexus.target.part and (
                                                    target_Nexus.target.part.Position - cam_position
                                            ).Magnitude or math.huge)
                                    )
                                    then
                                        closest_entry = player
                                        closest_part = part
                                        target_Nexus.target.distance = mouse_distance
                                    end
                                end
                            end
                        end
...
local function StopUndergroundAAAnimation()
    if ResolveUndergroundAA then
        local TargetPlayer = target_Nexus.target.entry
...
do -- update target
    if closest_part and (closest_part ~= target_Nexus.target.part) then
        target_Nexus.target.part = closest_part
        target_Nexus.target.entry = entry
        target_Nexus.target.distance = (closest_part.Position - camera.CFrame.Position).Magnitude
        ...
        highlight_player = target_Nexus.target.entry -- highlight

        do -- auto shoot
            if local_player.Character then
                if flags["Enable Auto Shoot"] and target_Nexus and target_Nexus.target and target_Nexus.target.entry and target_Nexus.target.part and player_esp.functions.is_visible2(entry) then
                    mouse1press()
                    mouse1release()
                end
            end            
        ...
        do -- aimbot
            if (target_Nexus and target_Nexus.target and target_Nexus.target.entry and target_Nexus.target.part and target_Nexus.target.entry ~= local_player) then
                if (flags["Enable Aimbot"] and flags["Aimbot Keybind"].active) then
                    local enemy_pos = target_Nexus.target.part.Position
...
library:change_fontsize(13) -- erm idk why i added this. anyways sizes for pixel fonts are: proggy (12), minecraftia (12), proggytiny (9), tahoma windows xp (12)
 notifications:create_notification({name = "Nexus Loaded!"})